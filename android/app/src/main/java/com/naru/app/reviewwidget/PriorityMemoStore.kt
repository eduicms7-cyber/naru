...
data class DueMemo(val id: String, val text: String, val color: String?, val imageUris: List<String>)
  ...
  fun load(context: Context): List<DueMemo> {
    ...
    return try {
      ...
      (0 until arr.length()).map { i ->
        ...
        val imageUris = if (obj.isNull("imageUris")) emptyList() else {
          val arrJson = obj.optJSONArray("imageUris")
          if (arrJson == null) emptyList() else (0 until arrJson.length()).map { j -> arrJson.getString(j) }
        }
        DueMemo(obj.getString("id"), obj.optString("text", ""), color, imageUris)
      }
    } catch (e: Exception) {
      emptyList()
    }
