...
- 구글 로그인: Supabase Auth의 Google OAuth 프로바이더를 그대로 사용(별도 네이티브 Google Sign-In SDK 아님 — 브라우저 리다이렉트 방식이라 Google Cloud Console에 "웹 애플리케이션" 타입 OAuth 클라이언트 하나면 충분하고, 패키지명/SHA-1 등록이 필요한 안드로이드 전용 클라이언트는 불필요). `flowType: 'pkce'`(`supabase.ts`)로 콜백이 `?code=...` 형태로 오게 하고, 웹은 `detectSessionInUrl`이 자동 처리, 네이티브는 `expo-web-browser`의 `openAuthSessionAsync`로 브라우저 세션을 열어 `app.json`의 `scheme`(`naru`)로 만든 딥링크(`naru://auth-callback`, `expo-linking`)로 돌아온 뒤 `exchangeCodeForSession`으로 직접 교환한다(`AuthContext.tsx`의 `signInWithGoogle`). **이 프로젝트는 `android/`가 커밋된 bare 워크플로우라 `app.json`의 `scheme`만으로는 안 됨** — `AndroidManifest.xml`의 `MainActivity`에 `naru` 스킴을 받는 `intent-filter`를 직접 추가해뒀음. Supabase 대시보드(Authentication → Providers → Google에 Google Cloud Console에서 만든 Client ID/Secret 입력, Google Cloud Console의 승인된 리디렉션 URI는 앱이 아니라 `https://<project-ref>.supabase.co/auth/v1/callback`)와 Supabase의 Redirect URLs(`naru://auth-callback` + 배포된 웹 URL)를 먼저 설정해야 실제로 동작한다 — 코드만으론 못 씀.
...
- `Todo { id, title, done, createdAt, completedAt?, tags?, isPinned?, dueDate? }` — 테이블: `todos`. `completedAt`은 `done`을 켤 때 채워지고 끄면 지워집니다. `isPinned`은 오늘 탭 목록 정렬용 별표(공지처럼 무조건 최상단). `dueDate`(선택, `YYYY-MM-DD`)는 할일 작성/수정 폼의 "기한 설정" 미니 달력으로 고른다 — 정렬 순서는 `TodayScreen.tsx`의 `visibleTodos`에 있음: ①고정 여부 ②미완료가 완료보다 먼저 ③미완료 안에서는 기한이 빠른 순 → 기한 없음(작성일 `createdAt` 내림차순), 완료는 완료일(`completedAt`) 기준 내림차순.
